...
/**@brief Function for adding Accelerometer characterstic to our service
 *
 * @param[in]   p_our_service        Our Service structure.
 *
 */
static uint32_t accel_char_add(ble_os_t * p_our_service)
{
...
/**@brief Function for adding Gyroscope characterstic to our service
 *
 * @param[in]   p_our_service        Our Service structure.
 *
 */
static uint32_t gyro_char_add(ble_os_t * p_our_service)
{
   // Add a custom characteristic UUID
   uint32_t            err_code;
   ble_uuid_t          char_uuid;
   ble_uuid128_t       base_uuid = BLE_UUID_OUR_BASE_UUID;
   char_uuid.uuid      = BLE_UUID_GYRO_CHARACTERISTIC_UUID;
   err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid.type);
   APP_ERROR_CHECK(err_code);
   ...
   err_code = sd_ble_gatts_characteristic_add(p_our_service->service_handle,
                                    &char_md,
                                    &attr_char_value,
                                    &p_our_service->char_gyro_handles);
   APP_ERROR_CHECK(err_code);
 ...
 *
 */
static uint32_t ambLight_char_add(ble_os_t * p_our_service)
{
   // Add a custom characteristic UUID
   uint32_t            err_code;
   ble_uuid_t          char_uuid;
   ble_uuid128_t       base_uuid = BLE_UUID_OUR_BASE_UUID;
   char_uuid.uuid      = BLE_UUID_AMBIENT_LIGHT_CHARACTERISTIC_UUID;
   err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid.type);
   APP_ERROR_CHECK(err_code);
   ...
   err_code = sd_ble_gatts_characteristic_add(p_our_service->service_handle,
                                    &char_md,
                                    &attr_char_value,
                                    &p_our_service->char_ambLight_handles);
   APP_ERROR_CHECK(err_code);
...
void our_service_init(ble_os_t * p_our_service)
{
  ...
  ambLight_char_add(p_our_service);
  accel_char_add(p_our_service);
  gyro_char_add(p_our_service);
}

/**
 * @brief Function to update Accelerometer characteristic.
 */
...
/**
 * @brief Function to update Gyroscope characteristic.
 */
void characteristic_gyro_update(ble_os_t *p_our_service, int16_t *value)
{
  if (p_our_service->conn_handle != BLE_CONN_HANDLE_INVALID)
  {
    ...
    hvx_params.handle = p_our_service->char_gyro_handles.value_handle;
    hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
    hvx_params.offset = 0;
    hvx_params.p_len  = &len;
    hvx_params.p_data = (uint8_t *)value;
...
/**
 * @brief Function to update temperature and humidity characteristic.
 */
...
/**
 * @brief Function to update ambient light characteristic.
 */
void characteristic_ambLight_update(ble_os_t *p_our_service, uint16_t *value)
{
  if (p_our_service->conn_handle != BLE_CONN_HANDLE_INVALID)
  {
    ...
    hvx_params.handle = p_our_service->char_ambLight_handles.value_handle;
    hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
    hvx_params.offset = 0;
    hvx_params.p_len  = &len;
    hvx_params.p_data = (uint8_t *)value;
